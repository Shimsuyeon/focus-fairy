...
| `/end`      | 집중 종료 (임계값 초과 시 확인 버튼 + 수동 입력: `/end 2시간 30분`)      |
| `/pause`    | 집중 일시정지 (휴식 시간 자동 차감)                                      |
| `/resume`   | 집중 재개                                                                |
| `/today`    | 오늘 팀원들의 집중 현황                                                  |
...
- **유저 타임존 지원** — Slack 프로필 타임존 기반 시간 표시 + 혼합 타임존 자동 감지
...
- **유저 타임존 지원** — Slack 프로필의 타임존 기반으로 시간 표시
  - 각 유저의 로컬 시간으로 `/start`, `/end`, `/pause`, `/resume` 메시지 표시
  - 혼합 타임존 자동 감지: 다른 시간대 유저가 감지되면 `(KST)`, `(ET)` 등 라벨 자동 표시
  - `/settings`에서 라벨 표시 모드 설정 (자동/항상/표시안함)
- **`/settings` 워크스페이스 설정 모달** — 워크스페이스별 설정 관리
  - 자동 기록 임계값 (시간) — `/end` 시 확인 버튼이 표시되는 기준
  - 기본 시간대 — 팀 집계에 사용되는 기준 시간대 (10개 주요 시간대)
  - 시간대 라벨 표시 모드 — 자동(혼합 시), 항상, 표시안함
- **`/end` "그대로 기록하기" 버튼** — 자동 기록 임계값 초과 시 확인 버튼 표시
  - 계산된 시간 그대로 기록하거나, `/end 2시간 30분`으로 수동 입력 가능

### 🔧 기술 개선

- `getUserName` → `fetchUserInfo` 리팩토링: 이름 + 타임존을 한 번의 API 호출로 캐싱
- `formatTime`에 `timezone`, `showLabel` 파라미터 추가
- `WorkspaceSettings` 타입 확장 (`defaultTimezone`, `timezoneLabelMode`, `mixedTimezones`)
- `TZ_LABELS` 매핑으로 Cloudflare Workers 환경에서 안정적인 타임존 약어 표시
...
- **`/pause` 일시정지** — 집중 세션을 일시정지하고 휴식
- **`/resume` 재개** — 일시정지된 세션을 다시 시작
- 휴식 시간은 `/end` 시 자동 차감되어 순수 집중 시간만 기록
- `/end` 메시지에 휴식 시간 표시 (예: `3시간 (휴식 30분 제외)`)

### 🔧 기술 개선

- Session 타입에 `pauseDuration` 필드 추가
- KV 체크인 데이터에 `pausedAt`, `totalPauseDuration` 저장
